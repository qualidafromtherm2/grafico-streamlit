...
if st.session_state["show_expander"]:
    with st.expander("Novo Registro", expanded=True):
        with st.form("registro_form"):
            ...
            # Incluímos a nova opção "TRI 300" no radio
            tri_modelo = st.radio("Modelo TRI", options=["TRI 380", "TRI 220", "TRI 300"])
...
# Parâmetros para a nova linha, por exemplo, "TRI 300"
default_tri300_min      = 16.0
default_tri300_max      = 22.0
default_tri300_tick     = 0.5
...
def gerar_grafico(
    ...
    ax1.set_title('Temperatura da água com escalas para TRI 380, TRI 220 e TRI 300')
    ax1.grid(True)
...
with col_tri:
    ...
    # NOVO: TRI 300
    row = st.columns([1, 1])
    row[0].markdown("**TRI 300 - Máximo**")
    tri300_max = row[1].number_input("", value=default_tri300_max, key="tri300_max", label_visibility="collapsed")
    row = st.columns([1, 1])
    row[0].markdown("**TRI 300 - Mínimo**")
    tri300_min = row[1].number_input("", value=default_tri300_min, key="tri300_min", label_visibility="collapsed")
    row = st.columns([1, 1])
    row[0].markdown("**TRI 300 - Intervalo**")
    tri300_tick = row[1].number_input("", value=default_tri300_tick, key="tri300_tick", label_visibility="collapsed")
