...
#   (removido o "marker='o'" e comentários de scatter para retirar pontos)
# --------------------------------------------------
def gerar_grafico(
    ...
    """Gera o gráfico base sem pontos, somente a linha azul."""
    temp = np.linspace(temp_min, temp_max, 13)
    x = np.linspace(pressao_inicial, pressao_final, 13)

    fig, ax1 = plt.subplots(figsize=(8,6))
    # Removida a marcação de pontos (marker='o')
    ...
    # Removendo a plotagem dos pontos de registro (scatter)
    # pois o objetivo é manter só a linha azul.
...
def gerar_grafico_sem_tri(pressao_inicial, pressao_final, temp_min, temp_max, tick_x, tick_y):
    """Gera um gráfico base (Temperatura Ambiente vs. Pressão baixa) sem pontos."""
    temp = np.linspace(temp_min, temp_max, 13)
    x = np.linspace(pressao_inicial, pressao_final, 13)

    fig, ax = plt.subplots(figsize=(8,6))
    # Removida a marcação de pontos por marker
    ...
    # Removendo a plotagem de pontos (scatter).
    # if "registros" in st.session_state:
    #     for reg in st.session_state["registros"]:
